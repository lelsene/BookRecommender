...
def get_content_based_recommendations(title, cosine_similarity=cosine_sim):
    ...
    return books.values[books.index.isin(books_indices)]
...
def get_collaborative_recommendations(fav_book, data=sparse_collaborative_books, n_recommendations=20, mapper=indices,
    ...
    # список рекомендаций
    rec_indices = []
    for index, distance in raw_recommends:
    ...
    # отбор списка названий книг по полученным индексам книг
    return books.values[books.index.isin(rec_indices)]


@app.route('/content_based', methods=["GET", "POST"])
def content_based():
    select_books = [(title.strip(), author) for title, author in zip(books['original_title'], books['authors'])]
    selected_cb = select_books[0][0]
    if request.method == "POST":
        selected_cb = request.form['content-based_select']
    content_based = [(book[21], book[9], book[10], book[7], int(book[8]), book[12]) for book in
                     get_content_based_recommendations(title=selected_cb)]
    return render_template('content_based.html', books=select_books, content_based=content_based,
...
def collaborative():
    select_books = [(title.strip(), author) for title, author in zip(books['original_title'], books['authors'])]
    selected_c = select_books[0][0]
    if request.method == "POST":
        selected_c = request.form['collaborative_select']
    collaborative = [(book[21], book[9], book[10], book[7], int(book[8]), book[12]) for book in
                     get_collaborative_recommendations(fav_book=selected_c)]
    return render_template('collaborative.html', books=select_books, collaborative=collaborative,
                           selected_c=selected_c)


@app.route('/top_books')
def top_books():
    legend = "Число человек, добавивших книгу в раздел [Прочитать позже]"
    values = list(books_to_read_top['title'].values)
    labels = list(books_to_read_top.index)
    return render_template('top_books.html', values=values, labels=labels, legend=legend)


@app.route('/top_genres')
def top_genres():
    values = list(new_tags['count'])
    labels = list(new_tags.index)
    return render_template('top_genres.html', values=values, labels=labels)


@app.route('/')
def main():
    return redirect("/content_based")
